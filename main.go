...
func generateExcelReport(data map[string]*model.SLOData) {
	...
	setCellWithStyle(f, "A1", fmt.Sprintf("SLO Report for %s\nList of SLOs that have never been below %g%% in %g days and 50%% of the total window has a negative error budget", providerInfo, *errorBudgetThreshold*100, window.Hours()/24), descriptionStyle)
	setCellWithStyle(f, "F1", "Generated by Vigil https://github.com/rluisr/vigil", descriptionStyle)
...
func setProperty(f *excelize.File) {
	err := f.SetDocProps(&excelize.DocProperties{
		Created:        time.Now().Format(time.RFC3339),
		Creator:        "Vigil",
		Description:    "Generated by Vigil https://github.com/rluisr/vigil",
		Identifier:     "xlsx",
