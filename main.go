...
func generateExcelReport(data map[string]*model.SLOData) {
	...
	setSheetView(f)
	setProperty(f)
	setCellWithStyle(f, "A1", fmt.Sprintf("SLO Report for %s\nList of SLOs that have never been below %g%% in %g days and 50%% of the total window has a negative error budget", *gcpProjectID, *errorBudgetThreshold*100, window.Hours()/24), descriptionStyle)
